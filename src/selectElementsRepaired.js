...
export async function selectElementsRepaired() {
    ...
    SELECT Distinct ?GUIDs ?label ?Storey WHERE { 
        ?ifcElement fog:hasIfcId-guid ?GUIDs ;
            nen2660:isImplementedBy ?elementNew .
        ?storey a bot:Storey ;
            rdfs:label ?Storey ;
            bot:hasElement ?ifcElement.        
        ?elementNew cto:isSubjectOfTask ?inspection ;
            rdfs:label ?label .
        ?inspection rii:chosenIntervention ?intervention .
        ?intervention a rii:Repair .
      }
    ORDER BY ?Storey ?label
      `, {
        sources: graphs,
    });
...
window.selectElementsRepaired = selectElementsRepaired;
selectElementsRepaired()
